const githubData = [
  ...
  const findID="8030403992";
  const githubDataLength = githubData.length;
  for (var i = 0; i < githubDataLength; i++ ) {
    if ( githubData[i].id === findID ) {
      for (var j = 0; j < githubData[i].payload.commits.length; j++) {
          console.log(`github message for push event id ${findID}: ${githubData[i].payload.commits[j].message}`);
      }
